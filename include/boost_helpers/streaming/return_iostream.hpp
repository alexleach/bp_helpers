...
namespace boost { namespace python {
  ...
  //
  // Add iostream<T> to the Boost Python Registry. This should be done on module
  // import, so we don't need to make a new HolderGenerator each time.
  // This puts the PyTypeObjects for iostream_base, istream, ostream and finally
  // iostream into the registry, assuming they are not there already.
  // It also registers to_python converters at the same time, and registers the
  // full type hierarchy whilst it's at it.
  template <class T>
  ...
  { 
      ...
      register_iostream_pytype<Pointee>();
      return iostream_t();
  }

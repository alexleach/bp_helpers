...
# else
        return PyUnicode_FromStringAndSize(ptr.get(), size);
# endif
    }
    //@}
...
# else
        return PyUnicode_FromFormat("%c", val);
# endif
    }
    //@}
...
# else
        if ((obj = PyUnicode_FromStringAndSize(NULL, size * count)) == NULL)
            return NULL;
...
# if PY_VERSION_HEX < 0x03000000
        ...
        p = PyString_AS_STRING(obj);
# else
        obj = PyUnicode_FromStringAndSize(NULL, right - left);
        p = PyUnicode_AS_UNICODE(obj);
# endif
